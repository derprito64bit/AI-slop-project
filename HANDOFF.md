...
| **Dashboard** (`/profile`) | Complete, and now the largest surface on the site: ten tools behind one shell — Overview, My list, Balance, Courses, Compare, Programs, Fields, Map, The data, Account. (Applications and Deadlines, the local-only application/deadline tracker, have been removed.) The Overview has a real empty state; see `DASHBOARD-NEXT.md` for what is still thin. |
